...
def get_simple_feature(test,label_field):
    """简单的几个特征,作为初学示例
    ...
    test = pd.merge(feature, pivot, on=keys, how='left')
    ...
    keys = ['User_id', 'Coupon_id']  # 主键
    prefixs = 'simple_' + '_'.join(keys) + '_'  # 特征名前缀,由label_field和主键组成
    pivot = pd.pivot_table(data[data['Date'].map(lambda x:str(x) != 'nan')], index=keys, values='cnt', aggfunc=len)  # 以keys为键,'cnt'为值,使用len统计出现的次数
    pivot = pd.DataFrame(pivot).rename(columns={
        'cnt': prefixs + 'received_and_consume_cnt'}).reset_index()  # pivot_table后keys会成为index,统计出的特征列会以values即'cnt'命名,将其改名为特征名前缀+特征意义,并将index还原
    feature = pd.merge(feature, pivot, on=keys, how='left')  # 将id列与特征列左连"""
    test = pd.merge(feature, pivot, on=keys, how='left')
    test.fillna(0,downcast='infer',inplace = True)
    ...
    #用户核销率
    """feature[prefixs + 'receive_and_consune_rate'] = list(map(
            lambda x ,y: x/y if y!=0 else 0,
            feature[prefixs + 'receive_and_consume_cnt'],
            feature[prefixs + 'receive_cnt']
            ))
    test[prefixs + 'receive_and_consune_rate'] = list(map(
            lambda x ,y: x/y if y!=0 else 0,
            test[prefixs + 'receive_and_consume_cnt'],
            test[prefixs + 'receive_cnt']
            ))"""
    ...
    #用户对领券商家15天内的核销数
    keys = ['User_id']
    pivot = pd.pivot_table(data[data['label'] == 1],index = keys,values='Merchant_id',
    ...
    #商家的券被核销率
    """feature[prefixs + 'received_and_consumed_rate'] = list(
    ...
    pivot = pd.pivot_table(data[data['label']==1],index = keys,values='cnt',
                           aggfunc = len)
    pivot = pd.DataFrame(pivot).rename(columns = {'cnt':prefixs+'receive_and_consume_cnt_15'}).reset_index()
    feature = pd.merge(feature, pivot, on=keys, how='left')
    feature.fillna(0,downcast='infer',inplace = True)
    test = pd.merge(feature, pivot, on=keys, how='left')
    test.fillna(0,downcast='infer',inplace = True)
    ...
    #优惠券15天内的核销率
#    feature[prefixs + 'received_and_consumed_rate_15'] = list(
#        map(lambda x,y: x/y if y!= 0 else 0,
#            feature[prefixs +'receive_and_consume_cnt_15'],
#            feature[prefixs +'received_cnt'])
#        )
#    test[prefixs + 'received_and_consumed_rate_15'] = list(
#        map(lambda x,y: x/y if y!= 0 else 0,
#            test[prefixs +'receive_and_consume_cnt_15'],
#            test[prefixs +'received_cnt'])
#        )
    #领取并消费优惠券的平均折扣率
    pivot = pd.pivot_table(data[data['Date'].map(lambda x:str(x) != 'nan')],index = keys,
                                values = 'discount_rate',aggfunc = np.mean)
    pivot = pd.DataFrame(pivot).rename(columns = {'discount_rate':prefixs+'get_and_consume_mean_discount'}).reset_index()
    feature = pd.merge(feature,pivot,on = keys,how = 'left')
    feature.fillna(0,downcast='infer',inplace = True)
    test = pd.merge(feature, pivot, on=keys, how='left')
    test.fillna(0,downcast='infer',inplace = True)
    ...
    #领取并消费优惠券的平均距离
    pivot  = pd.pivot_table(data[data['Date'].map(lambda x:str(x) != 'nan') & data['null_distance']==0],index = keys,
                           values = 'Distance',aggfunc = np.mean)
    pivot = pd.DataFrame(pivot).rename(columns = {'Distance':prefixs+'get_and_consume_mean_distance'}).reset_index()
    feature = pd.merge(feature,pivot,on = keys,how = 'left')
    feature.fillna(0,downcast='infer',inplace = True)
    test = pd.merge(feature, pivot, on=keys, how='left')
    test.fillna(0,downcast='infer',inplace = True)
    ...
    pivot = pd.DataFrame(pivot).rename(columns = {'Merchant_id':prefixs+'get_and_consume_differ_Merchaint_cnt'}).reset_index()
    feature = pd.merge(feature,pivot,on = keys,how = 'left')
    feature.fillna(0,downcast='infer',inplace = True)
    test = pd.merge(feature, pivot, on=keys, how='left')
    test.fillna(0,downcast='infer',inplace = True)
    ...
    #在多少不同商家领取的优惠券
    pivot  = pd.pivot_table(data,index = keys,
                                 values = 'Merchant_id',aggfunc = lambda x :len(set(x)))
    pivot = pd.DataFrame(pivot).rename(columns = {'Merchant_id':prefixs+'get_differ_Merchaint_cnt'}).reset_index()
    feature = pd.merge(feature,pivot,on = keys,how = 'left')
    feature.fillna(0,downcast='infer',inplace = True)
    test = pd.merge(feature, pivot, on=keys, how='left')
    test.fillna(0,downcast='infer',inplace = True)
...
#    feature['differ_Merchant_consume_rate'] = list(
#            map(lambda x ,y : x / y if y!=0 else 0,
#                feature[prefixs+'get_and_consume_differ_Merchaint_cnt'],
#                feature[prefixs+'get_differ_Merchaint_cnt'])
#            )
#    test['differ_Merchant_consume_rate'] = list(
#            map(lambda x ,y : x / y if y!=0 else 0,
#                test[prefixs+'get_and_consume_differ_Merchaint_cnt'],
#                test[prefixs+'get_differ_Merchaint_cnt'])
#            )
    
    
    #优惠券15天内被核销的平均时间间隔
    tmp = data[data['label'] == 1]
    tmp['gap'] = (tmp['date'] - tmp['date_received']).map(
            lambda x:x.total_seconds() / (60*60*24))
    pivot = pd.pivot_table(tmp,index = keys,values='gap',
                           aggfunc = np.mean)
    pivot = pd.DataFrame(pivot).rename(columns = {'gap':prefixs+'consumed_mean_time_gpa_15'}).reset_index()
    feature = pd.merge(feature, pivot, on=keys, how='left')
    feature.fillna(0,downcast='infer',inplace = True)
    test = pd.merge(feature, pivot, on=keys, how='left')
    test.fillna(0,downcast='infer',inplace = True)
...
def get_dataset(history_field, test, label_field):
    """构造数据集
    ...
    simple_feat ,test = get_simple_feature(test,label_field)  # 示例简单特征
...
if __name__ == '__main__':
    # 源数据
    off_train = pd.read_csv(r'C:\Users\ASUS\Desktop\群内竞赛\data\data\ccf_offline_stage1_train.csv')
    off_test = pd.read_csv(r'C:\Users\ASUS\Desktop\群内竞赛\data/ccf_offline_stage1_test_revised.csv')
    online = pd.read_csv(r'C:\Users\ASUS\Desktop\群内竞赛\data\data\ccf_online_stage1_train.csv')
    # 预处理
    ...
    # 构造训练集、验证集、测试集
    test,_ = get_dataset(test_history_field, test_middle_field, test_label_field)
    print('构造训练集')
    train ,test = get_dataset(train_history_field, test, train_label_field)
    print('构造验证集')
    validate,_ = get_dataset(validate_history_field, validate_middle_field, validate_label_field)
    print('构造测试集')

    #样本不均衡、
    a = train['label'].sum()/train.shape[0]
    # 线下验证
