...
def user_process(data_user,Train = 1): 
    ...
    if Train:
        ...
        data = data.drop(['arpu_201908',
       'arpu_201909', 'arpu_201910', 'arpu_201911', 'arpu_201912',
       'arpu_202001', 'arpu_202002', 'arpu_202003'],axis = 1)
...
def voc_process(data_voc):
    ...
    lo_data['cnt'] = 1
    lo_data.columns
    sum_of_call = pd.pivot_table(lo_data, index=["phone_no_m",'month'], values='cnt',
                        aggfunc=sum).reset_index(level = ["phone_no_m",'month'])
    avg_sum_of_call = pd.pivot_table(sum_of_call, index=["phone_no_m"], values='cnt',
    ...
    """
    lo_data.columns
    sum_of_call_time =( pd.pivot_table(lo_data, index=["phone_no_m",'month'], values='call_dur',
                        aggfunc=sum)/60).reset_index(level = ["phone_no_m",'month']) 
    
    sum_of_call_time = pd.pivot_table(sum_of_call_time, index=["phone_no_m"], values='call_dur',
                        aggfunc=np.mean).reset_index(level = ["phone_no_m"])
    data = pd.merge(data, sum_of_call_time, on='phone_no_m', how='left')
    
    """
    统计主叫次数，被叫次数，传呼的次数以及所占的比列
    """
    ...
    sum_of_call_calltype_id_1 =pd.pivot_table(lo_data, index=["phone_no_m",'month'], values='calltype_id_1',
                        aggfunc=sum).reset_index(level = ["phone_no_m",'month']) 
    sum_of_call_calltype_id_2 =pd.pivot_table(lo_data, index=["phone_no_m",'month'], values='calltype_id_2',
                        aggfunc=sum).reset_index(level = ["phone_no_m",'month']) 
    sum_of_call_calltype_id_3 =pd.pivot_table(lo_data, index=["phone_no_m",'month'], values='calltype_id_3',
                        aggfunc=sum).reset_index(level = ["phone_no_m",'month']) 
    sum_of_call_calltype_id_1 = pd.pivot_table(sum_of_call_calltype_id_1, index=["phone_no_m"], values='calltype_id_1',
                        aggfunc=np.mean).reset_index(level = ["phone_no_m"])
    
    sum_of_call_calltype_id_2 = pd.pivot_table(sum_of_call_calltype_id_2, index=["phone_no_m"], values='calltype_id_2',
                        aggfunc=np.mean).reset_index(level = ["phone_no_m"])
    sum_of_call_calltype_id_3 = pd.pivot_table(sum_of_call_calltype_id_3, index=["phone_no_m"], values='calltype_id_3',
                        aggfunc=np.mean).reset_index(level = ["phone_no_m"])
    ...
    sum_of_different_call = pd.pivot_table(lo_data, index=["phone_no_m","opposite_no_m"], values='sum_of_different_call',
                        aggfunc=sum).reset_index(level = ["phone_no_m","opposite_no_m"])
    
    sum_of_different_call['sum_of_different_call'] = 1
    sum_of_different_call = pd.pivot_table(sum_of_different_call, index=["phone_no_m"], values='sum_of_different_call',
    ...
    sum_of_different_imei = pd.pivot_table(lo_data, index=["phone_no_m","imei_m"], values='sum_of_different_imei',
                        aggfunc=sum).reset_index(level = ["phone_no_m","imei_m"])
    
    sum_of_different_imei['sum_of_different_imei'] = 1
    sum_of_different_imei = pd.pivot_table(sum_of_different_imei, index=["phone_no_m"], values='sum_of_different_imei',
                        aggfunc=sum).reset_index(level = ["phone_no_m"])
...
def sms_process(data_sms):
    ...
    时间处理
    """
    lo_data['request_datetime'] = pd.to_datetime(lo_data['request_datetime'], format="%Y/%m/%d %H:%M:%S")
    ...
    lo_data.columns
    sum_of_sms = pd.pivot_table(lo_data, index=["phone_no_m","month"], values='sum_of_sms',
                        aggfunc=sum).reset_index(level = ["phone_no_m","month"])
    sum_of_call_time = pd.pivot_table(sum_of_sms, index=["phone_no_m"], values='sum_of_sms',
                        aggfunc=np.mean).reset_index(level = ["phone_no_m"])
    ...
    sum_of_call_calltype_id_1 =pd.pivot_table(lo_data, index=["phone_no_m"], values='smstype_id_1',
                        aggfunc=sum).reset_index(level = ["phone_no_m"]) 
    sum_of_call_calltype_id_2 =pd.pivot_table(lo_data, index=["phone_no_m"], values='smstype_id_2',
                        aggfunc=sum).reset_index(level = ["phone_no_m"]) 
    ...
    """
    lo_data['cnt'] = 1
    sum_of_different_sms = pd.pivot_table(lo_data, index=["phone_no_m",'opposite_no_m'], values='cnt',
                        aggfunc=sum).reset_index(level = ["phone_no_m"])
    sum_of_different_sms['sum_of_different_sms'] = 1
    sum_of_different_sms = pd.pivot_table(sum_of_different_sms, index=["phone_no_m"], values='sum_of_different_sms',
                        aggfunc=sum).reset_index(level = ["phone_no_m"])
...
def app_process(data_app):
    ...
    """
#    lo_data.columns
#    sum_of_flow = pd.pivot_table(lo_data, index=["phone_no_m"], values='flow',
#                        aggfunc=sum).reset_index(level = ["phone_no_m"])
...
#    data = sum_of_flow.copy()
    ...
    每月平均流量
    """
    avg_of_flow = pd.pivot_table(lo_data, index=["phone_no_m",'month_id'], values='flow',
                        aggfunc=sum).reset_index(level = ["phone_no_m",'month_id'])
    
    avg_of_flow = pd.pivot_table(avg_of_flow, index=["phone_no_m"], values='flow',
                        aggfunc=np.mean).reset_index(level = ["phone_no_m"])
    data = avg_of_flow
    ...
    """
    lo_data['cnt'] = 1
    sum_of_app= pd.pivot_table(lo_data, index=["phone_no_m",'busi_name'], values='cnt',
                        aggfunc=sum).reset_index(level = ["phone_no_m",'busi_name'])
    sum_of_app['sum_of_app'] = 1
    sum_of_app= pd.pivot_table(sum_of_app, index=["phone_no_m"], values='sum_of_app',
                        aggfunc=sum).reset_index(level = ["phone_no_m"])
...
def train(data_notnull,test):
    ...
    data, label = oversampler.fit_sample(data, label)
    ...
    x_train,x_test,y_train,y_test = train_test_split(data,label,test_size=0.1,random_state=2020)
...
#                    feval=f1_score,
    ...
    预测
    """
    sub = pd.read_csv(r'E:\竞赛\诈骗电话识别\诈骗电话识别\诈骗电话号码识别-0527\submit_example.csv')
    ...
    sub.to_csv('subs.csv',index = None)
...
if __name__ == '__main__':
    ...
    test_app = pd.read_csv(r'E:\竞赛\诈骗电话识别\诈骗电话识别\诈骗电话号码识别-0527\test\test_app.csv')
    test_sms = pd.read_csv(r'E:\竞赛\诈骗电话识别\诈骗电话识别\诈骗电话号码识别-0527\test\test_sms.csv')
    test_voc = pd.read_csv(r'E:\竞赛\诈骗电话识别\诈骗电话识别\诈骗电话号码识别-0527\test\test_voc.csv')
    test_user = pd.read_csv(r'E:\竞赛\诈骗电话识别\诈骗电话识别\诈骗电话号码识别-0527\test\test_user.csv')
    
    
    
    data = data_concat(data_app,data_sms,data_voc,data_user,train = 1)
    values = dict([(col_name, col_mean) for col_name, col_mean in zip(data.columns.tolist(), data.mean().tolist())]) # 参看1，生成字典，key为列名，value为列对应的均值
    data.fillna(value=values, inplace=True)
    data_notnull = data
    
    test = data_concat(test_app,test_sms,test_voc,test_user,train = 0)
    
    values = dict([(col_name, col_mean) for col_name, col_mean in zip(test.columns.tolist(), test.mean().tolist())]) # 参看1，生成字典，key为列名，value为列对应的均值
