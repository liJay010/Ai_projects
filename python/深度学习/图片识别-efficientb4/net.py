"""
Code from GITHUB: https://github.com/lukemelas/EfficientNet-PyTorch
"""
import re
import math
import collections
...
def GetPretrainedNet(name):
    if name == 'efficientnet-b4':
        net = EfficientNet.from_pretrained('efficientnet-b4')
        net._fc = nn.Linear(1792, 45)
    elif name == 'efficientnet-b3':
        net = EfficientNet.from_pretrained('efficientnet-b3')
        net._fc = nn.Linear(1536, 45)
    else:
        # net = pretrainedmodels.__dict__[name](num_classes=1000, pretrained='imagenet')
        # net.last_linear = nn.Linear(1536, 45)
...
def cosine_decay(learning_rate, global_step, decay_steps, alpha=0.0):
    ...
    decayed_learning_rate = (learning_rate-alpha) * cosine_decay + alpha
    return decayed_learning_rate
def lr_sch(lr_start=0.001, warm_up_iters=2759*2,end_iters=2759*50,warm_up_lr=0.):
    def warm_up(base_lr, lr, cur_step, end_step):
        return base_lr + (lr - base_lr) * cur_step / end_step
    def get_learningrate(epoch):
        if epoch<warm_up_iters:
            lr = warm_up(warm_up_lr, lr_start, epoch, warm_up_iters)
        else:
            lr = cosine_decay(lr_start,epoch-warm_up_iters,end_iters-warm_up_iters,alpha=4e-5)
...
'''
class LabelSoothCrossEntropyLoss(nn.Module):
    def __init__(self, e=0.1, reduction='mean'):
        super(LabelSoothCrossEntropyLoss,self).__init__()
